...
function module_optimization() {
    SELECTION=$(whiptail --title "Optimisation & Laptop" --backtitle "$BACKTITLE" --checklist "Outils recommandés pour ThinkPad/Laptop :" 20 75 5 \
    "MICROCODE" "Intel Microcode (Sécurité/Stabilité CPU)" ON \
    "TLP" "TLP (Gestion Batterie & Énergie)" ON \
    "THERMALD" "Thermald (Gestion Chauffe CPU Intel)" ON \
    "POWERTOP" "Powertop (Diagnostic Conso)" OFF \
    3>&1 1>&2 2>&3)
